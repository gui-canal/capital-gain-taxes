<?php
    ...
    class TaxService implements StockOperationsInterface
    {
        ...
        {
            if (!$this->hadProfitsOnOperation($sellingOperation)) {
        ...
        private function applyProfitingTaxRules(OperationObject $sellingOperation): int
        {
            ...
            if ($liquidOperationProfit < 0) {
                $this->totalPreviousLosses = $liquidOperationProfit;

                return self::TAX_LESS_OPERATION_PRICE;
            }

            $this->totalPreviousLosses = 0;
            if ($liquidOperationProfit < self::TAX_LESS_PROFIT_LIMIT) {
                return self::TAX_LESS_OPERATION_PRICE;
            }
        ...
        {
            $this->totalPreviousLosses += $this->calculateOperationTotalPrice($sellingOperation);

            return self::TAX_LESS_OPERATION_PRICE;
        }

        private function hadProfitsOnOperation(OperationObject $sellingOperation): bool
        {
