<?php
    ...
    class OperationBuilder implements StockOperationsInterface
    {
        ...
        private function buildOperationObject(array $operation): OperationObject
        {
            ...
            if (!$this->isValidOperation($operation['operation'])) {
                throw new Exception("There were an invalid operation type");
            }

            if ($operation['unit-cost'] < 0 || $operation['quantity'] < 0) {
                throw new Exception("Operation`s quantity and unit cost must be bigger zero or higher");
            }
